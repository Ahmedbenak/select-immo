...
export default function PublierPage() {
  ...
  const [form,setForm] = useState({
    title:'', price_xof:'', listing_type:'location', property_type:'appartement',
    city:'Abidjan', commune:'', bedrooms:'', description:''
  });
    ...
    <section className="max-w-3xl mx-auto px-4 py-8">
      <div className="bg-white border rounded-2xl p-6 shadow-sm">
        ...
        <form onSubmit={onSubmit} className="grid gap-3">
          <input className="border rounded-lg px-3 py-2" placeholder="Titre"
                 onChange={e=>setForm(f=>({...f, title:e.target.value}))}/>

          <div className="grid grid-cols-2 gap-3">
            <input className="border rounded-lg px-3 py-2" placeholder="Prix (XOF)"
                   onChange={e=>setForm(f=>({...f, price_xof:e.target.value}))}/>
            <input className="border rounded-lg px-3 py-2" placeholder="Chambres"
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <select className="border rounded-lg px-3 py-2"
              ...
              <option>studio</option><option>terrain</option><option>bureau</option>
            </select>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <input className="border rounded-lg px-3 py-2" placeholder="Ville (ex: Abidjan)"
                   defaultValue="Abidjan" onChange={e=>setForm(f=>({...f, city:e.target.value}))}/>
            <input className="border rounded-lg px-3 py-2" placeholder="Commune (ex: Cocody)"
                   onChange={e=>setForm(f=>({...f, commune:e.target.value}))}/>
          </div>

          <textarea rows={5} className="border rounded-lg px-3 py-2" placeholder="Description"
